...
#            datavals.tofile(MPLout)
    ...
    def time_resample(self, timestep=[], starttime=[],endtime=[], datamethod = 'mean',verbose=False):
        ...
        for col in self.header:
            if starttime:
                self.header[col] = self.header[col][(self.header[col].index>=starttime)]
            if endtime:
                self.header[col] = self.header[col][(self.header[col].index<=endtime)]
        ...
        for n in range(self.header['numchans'][0]):    
            ...
            if starttime:
                self.data[n] = self.data[n][(self.data[n].index>=starttime)]
                   
            if endtime:
                self.data[n] = self.data[n][(self.data[n].index<=endtime)]
